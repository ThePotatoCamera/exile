...
if (_difficulty isEqualTo "hardcore") then
{
	...
	_vehicle = ["O_Truck_03_covered_F",[(_pos select 0) -30, (_pos select 1) -0],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
	_msgWIN = ['#0080ff',format ["Convicts have taken over the research camp, vehicle entry code is %1...",_pinCode]];
}
else
{
	_vehicle = ["O_Truck_03_covered_F",[(_pos select 0)-30,(_pos select 1)+0,0],[], 0, "CAN_COLLIDE"] call DMS_fnc_SpawnNonPersistentVehicle;
